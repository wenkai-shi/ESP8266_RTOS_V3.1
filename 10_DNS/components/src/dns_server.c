...
void dns_server_send(void)
{
    ...
    strcpy(dns_server_table.name, TABLENAME);       // TABLENAME是指定的域名
    ...
    if (rp != NULL)
    {
        ...
        ESP_LOGI(TAG, "the query_idx is %d", query_idx);
        ESP_LOGI(TAG, "the qry.addr is %02X", qry.addr);
...
{
    ESP_LOGI(TAG, "get the massage from sat");
    struct dns_ans_hdr hdr;
    upcb1 = upcb;
    addr1 = addr;
    port1 = port;
    if (p->tot_len < (SIZEOF_DNSANS_HDR + SIZEOF_DNSANS_HDRQUE))
    {
        ...
        ESP_LOGI(TAG, "dns_recv: pbuf too small");
    ...
    ESP_LOGI(TAG, "the length of q: %d", p->tot_len);
    ESP_LOGI(TAG, "the txid is: %02X and the questions number is %02X", txid, nquestions);

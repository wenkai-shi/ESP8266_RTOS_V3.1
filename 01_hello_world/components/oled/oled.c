...
uint8_t OLED_Write_Command(uint8_t OLED_Byte)
{
	...
	i2c_master_sentByte(0x78);			// 选择总线上的OLED[0111 100X B]
	i2c_master_Wait_Ack();

	i2c_master_sentByte(0x00);			// [0x00]表示下一字节写入的是[指令]
	i2c_master_Wait_Ack();

	i2c_master_sentByte(OLED_Byte);		// [指令代码]
	i2c_master_Wait_Ack();
...
uint8_t OLED_Write_Data(uint8_t OLED_Byte)
{
	...
	i2c_master_sentByte(0x40);			// [0x40]表示下一字节写入的是[数据]
	i2c_master_Wait_Ack();

	i2c_master_sentByte(OLED_Byte);		// [具体数据]
	i2c_master_Wait_Ack();

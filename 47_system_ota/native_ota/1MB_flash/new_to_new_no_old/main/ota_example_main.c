...
static bool connect_to_http_server()
{
    ...
    sock_info.sin_family = AF_INET;                             // 设定为ipv4
    sock_info.sin_addr.s_addr = inet_addr(EXAMPLE_SERVER_IP);   // 设定服务器IP
    sock_info.sin_port = htons(atoi(EXAMPLE_SERVER_PORT));      // 设定服务器端口号

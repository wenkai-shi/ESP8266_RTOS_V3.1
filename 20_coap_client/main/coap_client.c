#include <string.h>
#include <sys/socket.h>
#include <netdb.h>

#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/event_groups.h"

#include "esp_log.h"
#include "esp_wifi.h"
#include "esp_event.h"
#include "esp_netif.h"
#include "nvs_flash.h"

#include "protocol_examples_common.h"

#include "coap.h"

#define COAP_DEFAULT_TIME_SEC 5

/* 设置日志记录级别为0 */
#define COAP_LOGGING_LEVEL 0

/* The examples use uri "coap://californium.eclipse.org" that
   you can set via the project configuration (idf.py menuconfig)

   If you'd rather not, just change the below entries to strings with
   the config you want - ie #define COAP_DEFAULT_DEMO_URI "coap://californium.eclipse.org"
*/
#define COAP_DEFAULT_DEMO_URI CONFIG_TARGET_DOMAIN_URI

#define BUFSIZE 40

const static char *TAG = "CoAP_client";

static int resp_wait = 1;
static coap_optlist_t *optlist = NULL;
static int wait_ms;

static void message_handler(coap_context_t *ctx, coap_session_t *session, 
                coap_pdu_t *sent, coap_pdu_t *received, const coap_tid_t id)
{
    unsigned char* data = NULL;
    size_t data_len;
    coap_pdu_t *pdu = NULL;
    coap_opt_t *block_opt;
    coap_opt_iterator_t opt_iter;
    unsigned char buf[4];
    coap_optlist_t *option;
    coap_tid_t tid;

    // 确定响应代码类型
    if (COAP_RESPONSE_CLASS(received->code) == 2)
    {
        // 需要查看是否阻塞响应
        block_opt = coap_check_option(received, COAP_OPTION_BLOCK2, &opt_iter);
        if (block_opt)
        {
            uint16_t blktype = opt_iter.type;

            // 返回给定块选项 block_opt 中的字段 num 的值。
            if (coap_opt_block_num(block_opt) == 0) 
            {
                printf("Received:\n");
            }

            // 获取指定PDU的长度和数据指针
            if (coap_get_data(received, &data_len, &data)) 
            {
                printf("%.*s", (int)data_len, data);
            }

            if (COAP_OPT_BLOCK_MORE(block_opt)) 
            {
                // 创建下一个块请求的pdu
                pdu = coap_new_pdu(session);
                if (!pdu) {
                     ESP_LOGE(TAG, "set up coap_new_pdu failed");
                     goto clean_up;
                }
                ESP_LOGI(TAG, "set up coap_new_pdu succeed");

                pdu->type = COAP_MESSAGE_CON;               // 不可确认消息(一次性消息)
                pdu->tid = coap_new_message_id(session);    // 返回一个新的消息id并相应地更新会话
                pdu->code = COAP_REQUEST_GET;

                /* 从optlist中添加URI组件 */
                for (option = optlist; option; option = option->next ) 
                {
                    switch (option->number) 
                    {
                    case COAP_OPTION_URI_HOST :
                    case COAP_OPTION_URI_PORT :
                    case COAP_OPTION_URI_PATH :
                    case COAP_OPTION_URI_QUERY :
                        // 将指定类型的选项作为第一个传递给pdu
                        coap_add_option(pdu, option->number, option->length, option->data);
                        break;
                    default:
                        ;     // 跳过其他选项
                    }
                }

                // 将指定类型的选项作为第一个传递给pdu
                coap_add_option(pdu, blktype, coap_encode_var_safe(buf, sizeof(buf), ((coap_opt_block_num(block_opt) + 1) << 4) | COAP_OPT_BLOCK_SZX(block_opt)), buf);

                // 发送CoAP消息给指定的对等体
                tid = coap_send(session, pdu);

                if (tid != COAP_INVALID_TID) 
                {
                    resp_wait = 1;
                    wait_ms = COAP_DEFAULT_TIME_SEC * 1000;
                    return;
                }
            }
            printf("\n");
        } 
        else 
        {
            // 获取指定PDU的长度和数据指针
            if (coap_get_data(received, &data_len, &data)) 
            {
                printf("Received: %.*s\n", (int)data_len, data);
            }
        }
    }
clean_up:
    resp_wait = 0;
}

static void coap_example_task(void *p)
{
    struct hostent *hp;             // 定义一个网络数据库结构体
    struct ip4_addr *ip4_addr;

    coap_address_t    dst_addr, src_addr;   // 多功能地址结构体 sockaddr、sockaddr_in、sockaddr_in6
    static coap_uri_t uri;                  // URI的表示形式
    const char*       server_uri = COAP_DEFAULT_DEMO_URI;
    char* phostname = NULL;

    coap_set_log_level(COAP_LOGGING_LEVEL); // 设置日志级别为指定的值
    while (1) 
    {
        unsigned char _buf[BUFSIZE];
        unsigned char *buf;
        size_t buflen;
        int res;
        coap_context_t *ctx = NULL;
        coap_session_t *session = NULL;
        coap_pdu_t *request = NULL;

        optlist = NULL;
        // 将给定的字符串解析为URI组件
        if (coap_split_uri((const uint8_t *)server_uri, strlen(server_uri), &uri) == -1) 
        {
            ESP_LOGE(TAG, "CoAP server uri error");
            break;
        }
        ESP_LOGI(TAG, "CoAP server uri succeed");

        // URI的表示形式为COAPS且DTLS没有启用  或者  URI的表示形式为_TCP且TLS没有启用
        if ((uri.scheme == COAP_URI_SCHEME_COAPS && !coap_dtls_is_supported()) || 
            (uri.scheme == COAP_URI_SCHEME_COAPS_TCP && !coap_tls_is_supported())) 
        {
            ESP_LOGE(TAG, "CoAP server uri scheme error");
            break;
        }
        ESP_LOGI(TAG, "CoAP server uri scheme succeed");

        // 动态申请空间
        phostname = (char *)calloc(1, uri.host.length + 1);
        if (phostname == NULL) {
            ESP_LOGE(TAG, "calloc failed");
            continue;
        }
        ESP_LOGI(TAG, "calloc succeed");

        // 将URI的字符串数据拷贝到phostname
        memcpy(phostname, uri.host.s, uri.host.length);
        // 为 phostname 返回一个包含地址族af_inet的地址的表项
        hp = gethostbyname(phostname);
        // 释放 phostname
        free(phostname);

        if (hp == NULL) {
            ESP_LOGE(TAG, "DNS lookup failed");
            vTaskDelay(1000 / portTICK_PERIOD_MS);
            free(phostname);
            continue;
        }
        ESP_LOGI(TAG, "DNS lookup succeed");

        // 打印解析的IP
        ip4_addr = (struct ip4_addr *)hp->h_addr;
        ESP_LOGI(TAG, "DNS lookup succeeded. IP=%s", inet_ntoa(*ip4_addr));

        // 准备CoAP客户端socket
        coap_address_init(&src_addr);
        src_addr.addr.sin.sin_family      = AF_INET;        // 协议IPV4
        src_addr.addr.sin.sin_port        = htons(5683);       // 端口，转换为网络字节序(short int)
        src_addr.addr.sin.sin_addr.s_addr = INADDR_ANY;     // 本地地址

        if (uri.path.length) 
        {
            buflen = BUFSIZE;
            buf = _buf;

            // 将给定的URI路径分割成段
            res = coap_split_path(uri.path.s, uri.path.length, buf, &buflen);

            while (res--) 
            {
                // 将参数二添加到optlist中
                coap_insert_optlist(&optlist,
                    coap_new_optlist(COAP_OPTION_URI_PATH,      // 创建一个新的optlist条目
                    coap_opt_length(buf),                       // 返回buf长度
                    coap_opt_value(buf)));                      // 返回指向给定选项值的指针

                buf += coap_opt_size(buf);                      // 返回buf大小
            }
        }

        if (uri.query.length) {
            buflen = BUFSIZE;
            buf = _buf;

            // 将给定的URI查询分割成段
            res = coap_split_query(uri.query.s, uri.query.length, buf, &buflen);

            while (res--) 
            {
                // 将参数二添加到optlist中
                coap_insert_optlist(&optlist,
                    coap_new_optlist(COAP_OPTION_URI_QUERY,     // 创建一个新的optlist条目
                    coap_opt_length(buf),                       // 返回buf长度
                    coap_opt_value(buf)));                      // 返回指向给定选项值的指针

                buf += coap_opt_size(buf);                      // 返回buf大小
            }
        }

        // 创建一个新的coap_context_t对象，它将保存CoAP堆栈状态。
        ctx = coap_new_context(NULL);
        if (!ctx) {
           ESP_LOGE(TAG, "set up coap_new_context failed");
           goto clean_up;
        }
        ESP_LOGI(TAG, "set up coap_new_context succeed");

        // 准备CoAP服务器socket
        coap_address_init(&dst_addr);
        dst_addr.addr.sin.sin_family      = AF_INET;            // 协议IPV4
        dst_addr.addr.sin.sin_port        = htons(uri.port);    // 端口，转换为网络字节序(short int)
        dst_addr.addr.sin.sin_addr.s_addr = ip4_addr->addr;     // 地址

        // 创建到指定服务器的新客户机会话
        session = coap_new_client_session(ctx, &src_addr, &dst_addr,
           uri.scheme==COAP_URI_SCHEME_COAP_TCP ? COAP_PROTO_TCP :
           uri.scheme==COAP_URI_SCHEME_COAPS_TCP ? COAP_PROTO_TLS :
           uri.scheme==COAP_URI_SCHEME_COAPS ? COAP_PROTO_DTLS : COAP_PROTO_UDP);
        if (!session) {
           ESP_LOGE(TAG, "set up coap_new_client_session failed");
           goto clean_up;
        }
        ESP_LOGI(TAG, "set up coap_new_client_session succeed");

        // 注册一个新的消息处理程序，每当收到与正在进行的事务匹配的响应时就调用该处理程序
        coap_register_response_handler(ctx, message_handler);

        // 创建一个新的CoAP PDU
        request = coap_new_pdu(session);
        if (!request) {
           ESP_LOGE(TAG, "set up coap_new_pdu failed");
           goto clean_up;
        }
        ESP_LOGI(TAG, "set up coap_new_pdu succeed");

        request->type = COAP_MESSAGE_CON;
        request->tid = coap_new_message_id(session);    // 注册一个新的消息处理程序
        request->code = COAP_REQUEST_GET;
        // optlist 按照rfc7272排序后，添加到 PDU(request) 中
        coap_add_optlist_pdu(request, &optlist);

        resp_wait = 1;
        // 发送CoAP消息(request)给session
        coap_send(session, request);

        wait_ms = COAP_DEFAULT_TIME_SEC * 1000;

        while (resp_wait) 
        {
            int result = coap_run_once(ctx, wait_ms > 1000 ? 1000 : wait_ms);
            if (result >= 0) 
            {
               if (result >= wait_ms) 
               {
                  ESP_LOGE(TAG, "select timeout");
                  break;
               } 
               else 
               {
                  wait_ms -= result;
               } 
            }
        }
clean_up:
        if (optlist) {
            // 从 optlist 中删除所有条目，释放它们的内存使用
            coap_delete_optlist(optlist);
            optlist = NULL;
        }
        if (session) {
            // 会话上的递减引用计数器  注意，会话可能会因此被删除，并且在这个调用之后不应该被使用。
            coap_session_release(session);
        }
        if (ctx) {
            // 释放CoAP堆栈上下文
            coap_free_context(ctx);
        }
        coap_cleanup();
        // 只发送一次请求
        break;
    }

    vTaskDelete(NULL);
}

void app_main(void)
{
    ESP_ERROR_CHECK(nvs_flash_init());                  // 初始化默认的NVS分区   
    ESP_ERROR_CHECK(esp_netif_init());                  // 初始化tcpip适配器
    ESP_ERROR_CHECK(esp_event_loop_create_default());   // 创建默认事件循环

    ESP_ERROR_CHECK(example_connect());                 // 配置Wi-Fi或以太网，连接，等待IP

    xTaskCreate(coap_example_task, "coap", 5 * 1024, NULL, 5, NULL);
}

...
static void initialise_wifi(void)
{
	...
	ESP_ERROR_CHECK(esp_netif_init());						// 调用 tcpip_adapter_init() 初始化tcpip适配器
	...
	wifi_config_t wifi_config = {
		.sta = {
			...
			.password = EXAMPLE_WIFI_PASSWORD,		// wifi密码
			.rm_enabled = 1,						// 是否为连接启用无线电测量
			.btm_enabled = 1,						// 该连接是否启用了BTM
		},
	};
